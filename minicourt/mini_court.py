...
import constants
from utils import (convert_meters_to_pixel_distance, get_foot_position, get_closest_keypoint_index,
                   get_bounding_box_height, measure_xy_distance, convert_pixel_distance_to_meters, get_bbox_center)
...
class MiniCourt:
    ...
    def set_canvas_background_box_position(self, frame):
        ...
        self.end_x = frame.shape[1] - self.margin_court
        self.end_y = self.margin_court + self.drawing_rectangle_height
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self,
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            closest_player_id_to_ball = min(player_bbox.keys(), key=lambda x: measure_xy_distance(ball_position,
                                                                                                  get_foot_position(player_bbox[x])))
            ...
            for player_id, bbox in player_bbox.items():
                ...
                frame_index_maximum = min(len(player_boxes), frame_num + 50)
                bboxes_heights_in_pixels = [get_bounding_box_height(player_boxes[i][player_id]) for i in
                                            range(frame_index_minimum, frame_index_maximum)]
